...
# ==========================================================================
# MODE APERCU — previsualiser les trois mises en page sans emettre de signal
# ==========================================================================

def apply_preview(ctx, scenario: str):
    """Force le contexte sur un scenario d'affichage (OUVRIR / FERMER /
    MAINTENIR) sans toucher aux donnees ni au journal.

    Sert uniquement a verifier la mise en page des trois types d'email.
    Le sujet est prefixe [APERCU TEST] et rien n'est journalise.
    """
    ...
    if scenario == "OUVRIR":
        ctx.update(
            action="OUVRIR",
            directive="Investir : 50% SPY et 50% QQQ",
            spy_pct=50, qqq_pct=50, cash_pct=0,
            why=("Le momentum mensuel est favorable et le filtre de protection "
                 "15 jours est positif. La strategie se repositionne sur le marche."),
        )
    elif scenario == "FERMER":
        ctx.update(
            action="FERMER",
            directive="Sortir du marche : passer 100% en liquidites",
            spy_pct=0, qqq_pct=0, cash_pct=100,
            why=("Le filtre de protection 15 jours s'est declenche. La strategie "
                 "sort entierement du marche par securite."),
        )
    else:
        ctx.update(
            action="MAINTENIR",
            directive="Rien a faire : vous restez investi",
            spy_pct=50, qqq_pct=50, cash_pct=0,
            why=("Aucun changement de signal. Conservez vos positions."),
        )
    return ctx
...
def main():
    ...
    ap.add_argument("--preview", choices=["OUVRIR", "FERMER", "MAINTENIR"],
                    default=None,
                    help="Envoyer un email d'apercu du scenario choisi, "
                         "marque TEST, sans journal ni etat")
    args = ap.parse_args()
    ...
    # --- Apercu de mise en page : envoi marque TEST, aucune trace ------------
    if args.preview:
        ctx = apply_preview(ctx, args.preview)
        ...
        sujet = f"[APERCU TEST] {subject_for(ctx)}"
        send_email(sujet, html, sender, app_password, recipients, inline_png=png)
        print(f"Apercu {args.preview} envoye. Aucun journal, aucun etat modifie.")
        return
